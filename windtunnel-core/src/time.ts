...
export const timeModule = (mod: Module<SyncFn>): TimeReport => {
  const items = getEntries(mod);
  const results = timeItems(items);
  const report = createReport(results);
  return report;
};

const timeItems = (entries: Entry<SyncFn>[]): TimeResult[] => {
  return entries.map(timeItem);
};
